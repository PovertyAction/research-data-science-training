...
```bash
$ python readings.py --min data/inflammation-01.csv
0.0
0.0
1.0
...
...
```bash
$ python readings.py --max data/inflammation-01.csv
20.0
18.0
19.0
...
...
```bash
python readings-02.py --max data/inflammation-01.csv
```

but what if someone accidentally passes us a flag that isn't a recognized option?

```bash
python readings-02.py --median data/inflammation-01.csv
```
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        print('Usage: python readings-04.py action [file]')
        sys.exit(1)
...
```bash
python readings-04.py --max data/inflammation-01.csv
```
