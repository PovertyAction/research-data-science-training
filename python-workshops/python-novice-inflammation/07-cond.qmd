...
abstract: |
  Learn to implement decision-making logic in Python using conditional statements. Master if, elif, and else constructs, boolean operators, and conditional expressions for creating responsive programs.
date: last-modified
format:
  html: default
authors-ipa:
...
keywords: ["Python", "Conditionals", "Decision Making", "Boolean Logic", "Control Flow", "Tutorial"]
license: "CC BY 4.0"
---
...
![Flowchart diagram showing if-else construct testing if variable num is greater than 100](figures/python-flowchart-conditional.png)
...
conditional section.
![Flowchart diagram showing conditional section with multiple elif conditions](figures/python-else-if.png)
This contrasts with the case of multiple `if` statements, where every action can occur
as long as their condition is met.
![Flowchart diagram showing conditional section with multiple independent if statements](figures/python-multi-if.png)
:::
:::
...
There is a [built-in function `abs`](https://docs.python.org/3/library/functions.html#abs) that returns the absolute value of
a number:
