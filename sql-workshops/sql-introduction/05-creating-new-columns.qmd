...
abstract: |
  Create new columns in the query output. Rename columns in the query output. Use built-in functions to create new values. Use SQL syntax to conditionally create new values. Use SQL syntax to create a new column of 'binned' values.
date: last-modified
...
keywords: ["SQL", "Database", "Tutorial", "Data Science", "Columns", "Functions", "CASE"]
license: "CC BY 4.0"
---
...
Although in general text strings are manipulated in applications rather than the database, there are some simple string functions which are available in SQL which can sometimes be useful.
In the Farms table the A01\_interview\_date is a simple string. Like many dates in spreadsheet, it is represented as a dd/mm/yyyy string.
From the Dates lesson in spreadsheets we learned that this is not ideal as it leads to ambiguity.
We also learned that applications can convert dates from one format to another and perform date arithmetic such as calculating the differences between dates.

Unfortunately our dates in A01\_interview\_date column are text and not dates so we cannot do this directly.
We could certainly extract this table from DB Browser into a csv file and then use the spreadsheet techniques to manipulate them.
But this is a bit defeatist when we are trying to learn SQL. In fact SQL has several functions which can manipulate string values, such as extracting sub-strings from a long string.

## Using built-in functions to reformat existing values

All SQL implementations provide a range of built-in functions. The range varies between implementations although the commonly used functions are generally available.

The full list of functions available in SQLite is available from [here](https://sqlite.org/lang_corefunc.html).

The `substr` function allows you to extract a sub-string form a larger string. you have to specify the string (or column name), the first character position you wish to extract and the number of characters required

Using this function we can extract the different parts of the A01\_interview\_date string like this:
...
One use of dates is to sort data. The text string version of our dates will not sort correctly.
...
![](figures/SQL_05_dates_01.png){alt='Conversion failure'}
